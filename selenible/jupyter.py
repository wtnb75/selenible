...
class SelenibleKernel(Kernel):
    ...
    def cmd_thumbnail(self, args):
        "set thumbnail size"
        self.thumbnail = tuple(int(f) for f in args[:2])
    ...
    def do_execute(
        ...
        if isinstance(v, (list, tuple)):
            ...
            raise Exception(f"invalid type: {type(v)} : {v}")
